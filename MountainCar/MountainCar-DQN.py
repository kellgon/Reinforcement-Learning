...
def plot(df1, df2, df3, df4, chart_name, name, x, y):
	...
	plt.plot(exp1, label = "E Decay = .99")

	exp2 = df2.rolling(window=100).mean()
	plt.plot(exp2, label = "E Decay = .98")

	exp3 = df3.rolling(window=100).mean()
	plt.plot(exp3, label = "E Decay = .97")

	exp4 = df4.rolling(window=100).mean()
	plt.plot(exp4, label = "E Decay = .96")
...
if __name__ == '__main__':
	...
	plot(pd.DataFrame(rewards_list1), pd.DataFrame(rewards_list2),pd.DataFrame(rewards_list3),pd.DataFrame(rewards_list4),"MountainCar-DQN Epsilon Decay", "MountainCar-DQN Epsilon Decay", "Episode", "Reward")
